...
function renderPanelInner(c, entries, selected, sel, isRequesting, hasClient, onSelect, onClear, onClose, onReRequest) {
  ...
  return h("div", { style: { display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" } }, [
    // Header
    h("div", { style: { background: c.bg2, borderBottom: `1px solid ${c.border}`, padding: "10px 14px", display: "flex", alignItems: "center", gap: "12px", flexShrink: 0 } }, [
      h("span", { style: { fontSize: "13px", fontWeight: 700, letterSpacing: "0.5px" } }, "\u{1F6E1}\uFE0F Ciph Inspector"),
      h(
        "span",
        { style: { fontSize: "10px", background: c.bg3, border: `1px solid ${c.border}`, borderRadius: "10px", padding: "2px 8px", color: c.text2 } },
        `${entries.length} request${entries.length !== 1 ? "s" : ""}`
      ),
      h("span", { style: { fontSize: "10px", color: c.text2, background: "#0d2010", border: "1px solid #3fb95033", borderRadius: "6px", padding: "2px 6px" } }, "vue \u2726"),
      h("div", { style: { flex: 1 } }),
      h("button", { onClick: onClear, style: { padding: "3px 10px", borderRadius: "6px", border: `1px solid ${c.border}`, background: c.bg3, color: c.text2, cursor: "pointer", fontSize: "11px", fontFamily: "inherit" } }, "Clear"),
      h("button", { onClick: onClose, style: { padding: "3px 8px", borderRadius: "6px", border: `1px solid ${c.border}`, background: c.bg3, color: c.text2, cursor: "pointer", fontSize: "11px", fontFamily: "inherit" } }, "\u2715")
    ]),
    // Body row
    h("div", { style: { display: "flex", flex: 1, overflow: "hidden" } }, [
      // Log list
      h("div", { style: { width: "300px", borderRight: `1px solid ${c.border}`, overflowY: "auto", flexShrink: 0 } }, [
        h(
          "div",
        ...
        entries.length === 0 ? h("div", { style: { padding: "20px", color: c.text2, fontSize: "12px", textAlign: "center" } }, "No requests yet. Make an API call to see logs.") : entries.map((e, i) => {
          ...
          return h("div", {
            ...
            h("span", { style: { fontSize: "9px", fontWeight: 700, padding: "2px 4px", borderRadius: "4px", background: em.bg, color: em.text, textAlign: "center" } }, e.log.method),
            h("span", { style: { overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: c.text, fontSize: "11px" } }, e.log.route),
            h("span", { style: { color: statusColor(e.log.status), fontWeight: 600, fontSize: "11px" } }, e.log.status || "\u2026"),
            h("span", { style: { color: c.text2, fontSize: "10px" } }, `${e.log.duration}ms`)
          ]);
        })
      ]),
      // Detail pane
      h(
        ...
        !sel ? [h("div", { style: { display: "flex", alignItems: "center", justifyContent: "center", height: "100%", color: c.text2, fontSize: "13px" } }, "\u2190 Select a request to inspect")] : [
          h("div", { style: { marginBottom: "14px", paddingBottom: "10px", borderBottom: `1px solid ${c.border}` } }, [
            h("div", { style: { display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" } }, [
              h("span", { style: { fontSize: "10px", fontWeight: 700, padding: "2px 6px", borderRadius: "4px", background: mc.bg, color: mc.text } }, sel.log.method),
              h("span", { style: { fontWeight: 600, fontSize: "13px" } }, sel.log.route),
              h("span", { style: { fontWeight: 700, color: statusColor(sel.log.status) } }, sel.log.status || "\u2026"),
              h(
                "span",
          ...
          h(
            ...
            detailPairs.map(
              ([label, content]) => h("div", { key: label }, [
                h("div", { style: { fontSize: "10px", color: c.text2, textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "6px" } }, label),
                h(
                  "div",
                  { style: { background: c.bg2, border: `1px solid ${c.border}`, borderRadius: "6px", padding: "10px", overflowX: "auto" } },
                  h("pre", { style: { margin: 0, color: c.text, fontSize: "11px", lineHeight: "1.6", whiteSpace: "pre-wrap", wordBreak: "break-all" } }, content)
                )
              ])
            )
          ),
            ...
            encPairs.map(
              ([label, content]) => h("div", { key: label }, [
                ...
                h(
                  "div",
                  { style: { background: c.bg2, border: `1px solid ${c.border}`, borderRadius: "6px", padding: "10px", overflowX: "auto" } },
                  h("pre", { style: { margin: 0, color: c.text2, fontSize: "11px", lineHeight: "1.6", whiteSpace: "pre-wrap", wordBreak: "break-all" } }, content)
                )
              ])
            )
          )
...
var DevtoolsPanelComponent = defineComponent({
  ...
  setup(props) {
    ...
    const btnSnap = ref(null);
    const liveDragXY = ref(null);
    ...
    return () => {
      ...
      return h("div", {}, [
        // Docked panel
        isOpen && docked ? h("div", { style: { ...dockedPanelStyle.value, ...panelCommon, boxShadow: "0 0 40px rgba(0,0,0,0.6)", ...border } }, [resizeHandle, panelContent]) : null,
        // Floating panel
        isOpen && !docked ? h("div", { style: { ...floatingPanelStyle.value, ...panelCommon, border: `1px solid ${c.border}`, borderRadius: "10px", boxShadow: "0 8px 32px rgba(0,0,0,0.5)" } }, [panelContent]) : null,
        // Toggle button
        h("button", {
          ...
          wordmarkSvg,
          entrs.length > 0 ? h("span", { style: { background: "rgba(88,166,255,0.15)", color: "#58a6ff", border: "1px solid rgba(88,166,255,0.3)", borderRadius: "9999px", padding: "1px 7px", fontSize: "10px", fontFamily: "monospace", fontWeight: 600 } }, String(entrs.length)) : null
        ])
      ]);
    };
  }
...
//# sourceMappingURL=CiphDevtoolsPanel-IQE7ECEN.mjs.map